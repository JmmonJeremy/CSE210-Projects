...
// ### VARIABLE ATTRIBUTES ################################## // 
  ...
  private bool _isNumber; // for 1 of 3 conditions of StringNumberCheck method's while loops 
  private int _number; // for 2 of 3 conditions PosStringNumberCheck method's while loop & user's entry converted  
  private float _decimal; // for 2 of 3 conditions PosStringDeicimalCheck method's while loop & user's entry converted 
  ...
  private string _confirm; // condition for the ConfirmAnswer method's while loop & last check for other 3 methods StringDecimalCheck's

// ### CONSTRUCTORS ######################################### //  
  public Validator(string entry, string inputDirection)
  {    
    _isNumber = false; // set to run the StringNumberChec method's while loops    
    _number = 0; // set to run the StringNumberCheck while loop 
    _decimal = 0; // set to run the StringDecimalCheck while loop  
    ...
    _confirm = "no"; // set to run the ConfirmEntry, StringNumberCheck, PosStringDecimalCheck while loops
  }
...
// ### METHODS ############################################## // 
  ...
  public float PosStringDecimalCheck(string usePrompt, string confirm)
  {
    int cycle = 0; // shows if the while loop has cycled   
    string entry = "none";  // holds and returns the user's entry     
    _confirm = ""; // set so it won't restart the while loop if confirm != "Do ConfirmEntry"   
    while (!_isNumber || _decimal < 0 || _confirm == "no")
    {       
      cycle++;      
      ...
      // if it is a number it converts the string entry to an int _decimal 
      _isNumber = float.TryParse(entry, out _decimal);     
      if (!_isNumber || _decimal < 0) // if entry is not a number or is below 1
      {        
        Console.WriteLine();        
  ...
  public int PosStringNumberCheck(string usePrompt, string confirm, int minNumber)
  {    
    int cycle = 0; // shows if the while loop has cycled   
    string entry = "none";  // holds and returns the user's entry     
    _confirm = ""; // set so it won't restart the while loop if confirm != "Do ConfirmEntry"   
    while (!_isNumber || _number < minNumber || _confirm == "no")
    {       
      cycle++;      
      // DETERMINE WETHER TO PROMPT USER FOR INPUT
      if (usePrompt != "No prompt" || cycle > 1) // prompt the user for input
      ...
      if (!_isNumber || _number < minNumber) // if entry is not a number or is below minNumber
      {        
        Console.WriteLine();        
        Console.ForegroundColor = ConsoleColor.Green;      Console.Write(entry);        
        Console.ForegroundColor = ConsoleColor.Red;        Console.Write(" is not a valid number");        
  ...
  {  
    ...
    {        
      ...
      // using the PosStringNumberCheck method convert the string 
      // to an int to be used to run or end the while loop 
      // and stop a prompt question from being displayed the 1st time
      numberSelection = validator.PosStringNumberCheck("No prompt", "Don't ConfirmEntry", 1);
